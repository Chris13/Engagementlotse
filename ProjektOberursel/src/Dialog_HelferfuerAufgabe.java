...
public class Dialog_HelferfuerAufgabe extends JDialog {
	...
	public Dialog_HelferfuerAufgabe(JDialog parent,String aufgabenId, String aufgabenName, String projektId) {
		super(parent,true);
		projektid = projektId;
		aufgabenid = aufgabenId;
		helferName = aufgabenName;
		...
		contentPanel.setLayout(new BorderLayout(0, 0));
		{
			...
			{
				JLabel lblNewLabel = new JLabel("<html><u>Helferliste</u></html>");
	...
	private void einsaetzeAnzeigen()
	{
		...
		CS_DataBaseConnect.dbQuery(CS_SqlAbfragen.einsatzfuerhelferanzeigensql(aufgabenid),true);
		...
		CS_JTableUhrzeitButtonEditor editor = new CS_JTableUhrzeitButtonEditor(new JCheckBox(), Dialog_HelferfuerAufgabe.this);
		editor.setDialog(Dialog_HelferfuerAufgabe.this);
						...
						{	
							...
							{	
								if (CS_DataBaseConnect.dbQuery("SELECT max(Einsatz_id) FROM Einsatz",false))
								{
									...
									uebergabeWerte.add(tmp);
									if(CS_SqlAbfragen.einsatzzeitanlegensql(uebergabeWerte,mdlEinsatzID.getValueAt(0, 0).toString()))
										einsaetzeAnzeigen();				
								}
								else 
	...
	{		
		traegerermitteln();
		mglEinsatz = new JTable();
		CS_DataBaseConnect.dbQuery(CS_SqlAbfragen.helferaufgabensql(aufgabenid, projektid),true);
		...
		mglEinsatz.getColumnModel().getColumn(0).setHeaderValue("Nachname");				
		mglEinsatz.getColumnModel().getColumn(1).setHeaderValue("Vorname");
		mglEinsatz.getColumnModel().getColumn(2).setHeaderValue("EMailadresse");	
		mglEinsatz.getColumnModel().getColumn(3).setHeaderValue("Telefonnummer");
		mglEinsatz.getColumnModel().getColumn(4).setHeaderValue("Träger");
		mglEinsatz.getColumnModel().getColumn(4).setCellEditor(new DefaultCellEditor(cbFaehigkeiten));
		mglEinsatz.getColumnModel().getColumn(5).setHeaderValue("Beginn Datum");
		mglEinsatz.getColumnModel().getColumn(6).setHeaderValue("Ende Datum");
		mglEinsatz.getColumnModel().getColumn(6).setCellEditor(editor);
		mglEinsatz.getColumnModel().getColumn(7).setHeaderValue("Einsatzzeit");
		/*
		mglEinsatz.getColumnModel().getColumn(7).setCellEditor(new CS_JComboButtonCellEditor(Dialog_HelferfuerAufgabe.this));
		*/
		mglEinsatz.getColumnModel().getColumn(8).setHeaderValue("Notiz");
		mglEinsatz.getColumnModel().getColumn(8).setCellRenderer(new CS_JTableNotizButtonRenderer());
		editor = new CS_JTableNotizButtonEditor(new JCheckBox());
		editor.setDialog(Dialog_HelferfuerAufgabe.this);		
		mglEinsatz.getColumnModel().getColumn(5).setCellEditor(new CS_JDateChooserCellEditor());
		mglEinsatz.getColumnModel().getColumn(6).setCellEditor(new CS_JDateChooserCellEditor());
		mglEinsatz.getColumnModel().getColumn(9).setHeaderValue("Anlegen");
		mglEinsatz.getColumnModel().getColumn(9).setCellRenderer(mglEinsatz.getDefaultRenderer(Boolean.class));
		mglEinsatz.getColumnModel().getColumn(9).setCellEditor(mglEinsatz.getDefaultEditor(Boolean.class));   
		...
		mglEinsatz.getColumnModel().getColumn(4).setPreferredWidth(180);
		mglEinsatz.getColumnModel().getColumn(7).setPreferredWidth(180);
		...
		mglEinsatz.removeColumn(mglEinsatz.getColumnModel().getColumn(10));
